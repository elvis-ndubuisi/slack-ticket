...
async function wrapAction(fn: () => Promise<void>) {
  try {
    ...
    const exitCode =
      error instanceof CLIError || typeof error.exitCode === 'number' ? error.exitCode : 1;
...
program
  ...
  .action((slackUrl: string, options) =>
    wrapAction(async () => {
...
program
  ...
  .action((issueNumber: string, slackUrls: string[], options) =>
    wrapAction(async () => {

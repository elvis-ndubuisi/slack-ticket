...
function makeRequest(
    ...
    return new Promise((resolve, reject) => {
            ...
            (res) => {
                ...
                res.on('end', () => {
                    if (res.statusCode === 401 || res.statusCode === 403) {
                        const err = new Error(`AI API authentication failed (HTTP ${res.statusCode}). Check your API key.`);
                        (err as any).exitCode = 3;
                        reject(err);
                        return;
                    }
                    if (res.statusCode && res.statusCode >= 400) {
                        if (res.statusCode === 429) {
                            ...
                            reject(new CLIError(`AI provider authentication failed (HTTP ${res.statusCode}). Check your API key.`, 3));
                        } else {
        ...
        const timer = setTimeout(() => {
            req.destroy();
            reject(new CLIError(`AI request timed out after ${timeoutMs}ms. Consider increasing 'ai.timeoutMs' in your config.`, 3));
        }, timeoutMs);

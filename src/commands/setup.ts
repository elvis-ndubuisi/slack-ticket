...
export async function runSetup(): Promise<void> {
    ...
    const timeoutInput = await input({
        ...
        default: '20000',
        validate: (v: string) => (!isNaN(Number(v)) && Number(v) > 0 ? true : 'Must be a positive number'),
    });
    ...
    spinner.stopAndPersist({
        ...
        text: aiOk ? `AI: validated (${provider} / ${aiModel})` : `AI: ${chalk.red('validation failed')}`,
    });
    ...
    console.log(`  Slack:        ${slackOk ? chalk.green('validated ✓') : chalk.red('not validated ✗')}`);
    console.log(
        `  GitHub:       ${githubUserOk && githubRepoOk

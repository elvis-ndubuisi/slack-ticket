...
import { resolveLabels, filterExistingLabels, assembleIssueBody, createIssue, postImageComment, addToProject } from '../github.js';
...
export async function runCreate(slackUrl: string, options: Record<string, any>): Promise<void> {
    ...
    const thread = await fetchThread(parsedUrl, depth, config.slack.botToken, !options.noImage && config.defaults.imageHandling);
    ...
    const issue = await createIssue(owner, repo, aiOutput.title, finalBody, labels, config.github.token);
    ...
    if (thread.imageInfo.downloaded) {
        ...
        await postImageComment(owner, repo, issue.number, thread.imageInfo.downloaded.filePath, thread.imageInfo.downloaded.filename, config.github.token);
...
function printPreview(p: {
    ...
    if (p.imageInfo.totalCount > 1) {
        console.log(chalk.yellow(`  ⚠  ${p.imageInfo.totalCount} images found. Only the first will be attached.`));
